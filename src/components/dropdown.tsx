...
const Dropdown = ({list,title,selected, setState} : {list : string[],selected:string, title  : string,setState:Dispatch<SetStateAction<string>>}) => {
  ...
  return (
    <div className="relative cursor-pointer inline-block mt-2 mx-4">
      <ButtonEl endIcon={!open ?  <DropdownIcon dim="30"  /> : <DropUpIcon dim="30" /> } particularStyle={" w-30 mx-0 "} placeholder={selected === "blank" ? title : selected} onClickHandler={funSetOpen} buttonType={"dropdown"} />

      {open && (
        <div className="absolute mt-2 w-50 h-30 overflow-y-auto scrollbarSB rounded-md shadow-lg bg-white ring-1 ring-black/5 z-10">
          <div className="py-1">
